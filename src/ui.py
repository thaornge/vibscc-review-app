...
def annotation_form(prefix: str, initial: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if eligibility == "REMOVE":
        ...
        remove_reason = st.radio("Remove reason *", REMOVE_REASON_OPTIONS, index=REMOVE_REASON_OPTIONS.index(remove_value), horizontal=True, key=f"{prefix}_remove")
        c_label = s_label = a_label = None
    else:
        c_col, s_col, a_col = st.columns([1.1, 2, 2.2])
        c_options = C_OPTIONS
        with c_col:
            c_label = st.radio("C label", c_options, index=c_options.index(initial.get("C_label", "C0")), horizontal=True, key=f"{prefix}_c")
        ...
        with s_col:
            s_label = st.radio("S label", s_options, index=s_options.index(s_value), horizontal=True, key=f"{prefix}_s")
        with a_col:
            a_label = st.radio("A label", a_options, index=a_options.index(a_value), horizontal=True, key=f"{prefix}_a")
        remove_reason = None
    ...
    with evidence_col:
        evidence = st.text_area("Evidence", value=initial.get("evidence") or "", height=68, key=f"{prefix}_evidence")
    with note_col:
        note = st.text_area(note_label, value=initial.get("note") or "", height=68, key=f"{prefix}_note")
